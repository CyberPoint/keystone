...
export const Platform = () => {
  ...
  const platformList = useAppSelector(state => state.platform.entities);
  const loading = useAppSelector(state => state.platform.loading);
  const totalItems = useAppSelector(state => state.platform.totalItems);
  ...
  return (
    <div>
      <h2 id="platform-heading" data-cy="PlatformHeading">
        <Translate contentKey="keyStoneApp.platform.home.title">Platforms</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="keyStoneApp.platform.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/platform/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="keyStoneApp.platform.home.createLabel">Create new Platform</Translate>
          </Link>
        </div>
      </h2>
        ...
        {platformList && platformList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="keyStoneApp.platform.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th className="hand" onClick={sort('accessLevel')}>
                  ...
                  <FontAwesomeIcon icon={getSortIconByFieldName('accessLevel')} />
                </th>
            ...
            <tbody>
              {platformList.map((platform, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/platform/${platform.id}`} color="link" size="sm">
                      {platform.id}
                    </Button>
                  </td>
                  <td>{platform.name}</td>
                  ...
                  <td>{platform.added ? <TextFormat type="date" value={platform.added} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{platform.updated ? <TextFormat type="date" value={platform.updated} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{platform.active ? 'true' : 'false'}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/platform/${platform.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                      ...
                      <Button
                        tag={Link}
                        to={`/platform/${platform.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`/platform/${platform.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="keyStoneApp.platform.home.notFound">No Platforms found</Translate>
            </div>
          )
        )}
      </div>
      {totalItems ? (
        <div className={platformList && platformList.length > 0 ? '' : 'd-none'}>
